...
function buildTrackAPrompt(question, articleText) {
    ...
    // TEMPORAL AWARENESS - EDUCATION NOT SNAPSHOT
    prompt += '## YOUR TRAINING IS AN EDUCATION, NOT A SNAPSHOT OF REALITY\n\n';
    prompt += '**TODAY IS: ' + currentDate + ' (' + isoDate + ')**\n\n';
    prompt += 'Think of your training data the way a human thinks about their schooling — it taught you how to think, ';
    prompt += 'gave you historical context, and built your understanding of patterns. But it is NOT a live feed of current reality.\n\n';
    prompt += '### CRITICAL TEMPORAL RULES\n\n';
    prompt += '**BEFORE making ANY claim about:**\n';
    prompt += '- Whether someone is ALIVE or DEAD\n';
    prompt += '- Who CURRENTLY holds a government position or title\n';
    prompt += '- What RECENTLY happened (past 2 years)\n';
    prompt += '- Current status of ANY ongoing situation\n\n';
    prompt += '**YOU MUST:**\n';
    prompt += '1. ASSUME your training data is OUTDATED — people die, get fired, get appointed, resign\n';
    prompt += '2. SEARCH FIRST to verify current reality before making any factual claim\n';
    prompt += '3. If you cannot verify, SAY SO explicitly — do NOT guess or assume nothing changed\n';
    prompt += '4. NEVER say "X is alive" or "X is the current [position]" based on training data alone\n\n';
    prompt += '**THE FAILURE MODE:**\n';
    prompt += 'Assuming your training data reflects current reality, then confidently stating outdated information as fact.\n';
    prompt += 'Example: Saying "Rob Reiner is alive" when he was murdered a week ago, or "Christopher Wray is FBI Director" ';
    prompt += 'when Kash Patel was confirmed months ago. This destroys VERITAS credibility instantly.\n\n';
    prompt += '**CORRECT BEHAVIOR:**\n';
    prompt += '1. See claim about current status → 2. Search to verify → 3. State verified facts → 4. If unverifiable, acknowledge uncertainty\n\n';
