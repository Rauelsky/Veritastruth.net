...
<body>
    ...
    <header class="site-header">
        <div class="header-content">
            ...
            <div class="version-badge">v4.2 — Track A</div>
        </div>
    </header>
...
Example: 'The Earth is approximately 4.5 billion years old'" oninput="updateCharCounter()"></textarea>
    ...
    <script>
        ...
        function formatCompareContent(content, sectionKey) {
            ...
            // Special handling for realityFactors - handle BOTH object format (Initial) and array format
            if (sectionKey === 'realityFactors') {
                // Check if it's the object format (from Initial Assessment)
                if (typeof content === 'object' && !Array.isArray(content)) {
                    var html = '<div style="margin: 0; padding: 0;">';
                    
                    // Expected properties from Initial Assessment
                    var factors = [
                ...
                // Array format (legacy or alternative)
            ...
            if (sectionKey === 'evidenceAnalysis' && typeof content === 'object' && !Array.isArray(content)) {
                var html = '';
                if (content.whatSupportsTheClaim) {
                    html += '<p style="margin-bottom: 12px;"><strong style="color: var(--score-positive);">What Supports the Claim:</strong> ' + content.whatSupportsTheClaim + '</p>';
                }
                ...
                if (content.whatRemainsGenuinelyUncertain) {
                    html += '<p style="margin-bottom: 12px;"><strong style="color: var(--text-muted);">What Remains Uncertain:</strong> ' + content.whatRemainsGenuinelyUncertain + '</p>';
                }
                // Handle other common evidence analysis fields
                if (content.supporting && Array.isArray(content.supporting)) {
                    html += '<p style="margin-bottom: 8px;"><strong>Supporting Evidence:</strong></p><ul style="margin-left: 16px;">';
                    content.supporting.forEach(function(item) { html += '<li>' + item + '</li>'; });
                    html += '</ul>';
                }
                if (content.contradicting && Array.isArray(content.contradicting)) {
                    html += '<p style="margin-bottom: 8px;"><strong>Contradicting Evidence:</strong></p><ul style="margin-left: 16px;">';
                    content.contradicting.forEach(function(item) { html += '<li>' + item + '</li>'; });
                    html += '</ul>';
                }
                return html || formatGenericContent(content);
            }
            ...
            if ((sectionKey === 'frameworkAnalysis' || sectionKey === 'perspectiveAnalysis') && typeof content === 'object' && !Array.isArray(content)) {
                var html = '';
                if (content.complementaryInsight) {
                    html += '<p style="margin-bottom: 12px;"><strong style="color: var(--accent-cyan);">Complementary Insight:</strong> ' + content.complementaryInsight + '</p>';
                }
                if (content.bridgingWisdom) {
                    html += '<p style="margin-bottom: 12px;"><strong style="color: var(--accent-purple);">Bridging Wisdom:</strong> ' + content.bridgingWisdom + '</p>';
                }
                // Handle other common framework fields
                for (var key in content) {
                    if (content.hasOwnProperty(key) && !['complementaryInsight', 'bridgingWisdom'].includes(key)) {
